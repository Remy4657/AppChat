...
export const sendFriendRequest = async (data) => {
    ...
    const resultRequestFrom = await FriendRequest.findById(createdRequest._id)
        ...
        .lean();
    return { resultRequestTo, resultRequestFrom, notificationContent }
}
export const acceptFriendRequest = async (data) => {
    ...
    if (friendRequest.to._id.toString() !== userId.toString()) {
        throw new ForbiddenError("Bạn không có quyền chấp nhận lời mời này")
    }

    await Friend.create({
        userA: friendRequest.from._id,
        userB: friendRequest.to._id,
    });
    const notificationContent = `${friendRequest.to.displayname || friendRequest.to.username} đã chấp nhận lời mời kết bạn của bạn.`;

    const notification = await Notification.create({
        userId: friendRequest.from._id,
        actorId: friendRequest.to._id,
        type: 'friend_accept',
        content: notificationContent,
        is_read: false
    });
    const { userId: x, actorId: y, ...formatedNotification } = notification.toObject()

    await FriendRequest.findByIdAndDelete(requestId);

    return { friendRequest, formatedNotification }
}
export const declineFriendRequest = async (data) => {
    const { requestId, userId } = data;

    const friendRequest = await FriendRequest.findById(requestId).populate("from", "_id username displayname avatarUrl").lean()
...
export const getFriendRequests = async (userId) => {
    ...
    return { sent, received, allUsers: usersWithFlags };
}