...
export const checkFriendship = async (req, res, next) => {
    ...
    if (!recipientId && memberIds.length === 0) {
        throw new BadRequestError("Cần cung cấp recipientId hoặc memberIds");
    }
    ...
    if (recipientId) {
        ...
        if (!isFriend) {
            throw new ForbiddenError("Bạn chưa kết bạn với người này");
        }
    ...
    if (notFriends.length > 0) {
        throw new ForbiddenError("Bạn chỉ có thể thêm bạn bè vào nhóm.");
    }
...
export const checkGroupMembership = async (req, res, next) => {
    ...
    if (!isMember) {
        throw new ForbiddenError("Bạn không ở trong group này.");
    }
